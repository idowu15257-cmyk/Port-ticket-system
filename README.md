...
3. **Track My Tickets**: View previously submitted complaints from the operator dashboard
4. **Use Conversation Thread**: Original complaint is shown as first message and both operator/technician names are visible in replies
5. **Reply After Technician Response**: Operators can continue the chat once the technician has responded
...
4. **Urgent Signal**: Urgent tickets are visually highlighted before opening details
5. **Update Status**: Change status to "In Progress" or "Resolved"
...
- Public users can no longer change backend URL from the login screen.
- Backend API URL is now managed in admin settings after login.
- For iPhone users today, publish the frontend on HTTPS and use Safari "Add to Home Screen" (PWA-style access).
- Native iOS App Store delivery is a separate phase that requires Capacitor iOS setup on macOS/Xcode plus Apple signing.
- This project auto-applies Java 17 compatibility patching for Capacitor Android modules via `npm run java:compat` (also run automatically on `npm install`, `cap:sync`, and APK build scripts).
