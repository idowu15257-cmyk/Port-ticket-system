...
## 📱 iPhone Access (Fastest Path)

1. Host frontend over HTTPS (Vercel/Netlify or any SSL host)
2. Point backend API URL once from admin settings
3. On iPhone Safari, open app URL and tap **Share > Add to Home Screen**
4. Users can launch the app like a native icon (PWA-style)

Native iOS build via App Store/TestFlight is optional later and requires Capacitor iOS + Xcode on macOS.
...
### Issue: "Cannot change server URL from login"
**Solution**: This is expected in distribution builds. Login as admin and use **User Admin > Server Settings**.
