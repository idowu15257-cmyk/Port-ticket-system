...
app.post('/api/tickets', verifyToken, async (req, res) => {
  ...
  if (!String(title || '').trim() || !String(description || '').trim() || !String(location || '').trim()) {
    return res.status(400).json({ error: 'Title, description, and RTG/QC are required' });
  }

  if (priority && !allowedPriorities.includes(priority)) {
    return res.status(400).json({ error: 'Priority must be low, medium, or urgent' });
  }
...
const canReadAnyTicket = (req) => req.user.role === 'technician' || req.user.role === 'admin';

const canAccessTicket = (req, ticket) => {
  if (!ticket) return false;
  if (canReadAnyTicket(req)) return true;
  return req.user.role === 'operator' && ticket.created_by === req.user.id;
};
...
app.get('/api/tickets', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!canReadAnyTicket(req)) {
      if (req.user.role !== 'operator') {
        return res.status(403).json({ error: 'Not authorized to view tickets' });
      }
...
app.get('/api/tickets/:id', verifyToken, async (req, res) => {
  try {
    ...
    const hasTechnicianReply = comments.some((comment) => {
    ...
    res.json({
      ...ticketWithUsers,
      comments,
      can_operator_reply: req.user.role !== 'operator' ? true : hasTechnicianReply
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/tickets/:id/comments', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const isOperatorOwner = req.user.role === 'operator' && ticket.created_by === req.user.id;

    if (!isTechOrAdmin && !isOperatorOwner) {
      return res.status(403).json({ error: 'Not allowed to reply to this complaint' });
    }

    if (isOperatorOwner) {
      const { data: existingComments, error: commentsError } = await supabase
        .from('ticket_comments')
        .select('id, user:users(role)')
        .eq('ticket_id', req.params.id);

      if (commentsError) throw commentsError;

      const hasTechnicianReply = existingComments.some((comment) => {
        const role = comment.user?.role;
        return role === 'technician' || role === 'admin';
      });

      if (!hasTechnicianReply) {
        return res.status(403).json({ error: 'You can reply after a technician responds first' });
      }
    }
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        is_internal: isTechOrAdmin ? isInternal || false : false
      }])
      .select('*, user:users(full_name, email, role)');
...
app.post('/api/tickets/:id/comments/:commentId/upload', verifyToken, upload.array('files', 5), async (req, res) => {
  try {
    ...
    const canUpload = req.user.role === 'technician' || req.user.role === 'admin' || ticket.created_by === req.user.id;
    if (!canUpload) {
      return res.status(403).json({ error: 'Not allowed to upload files for this response' });
    }
...
app.get('/api/tickets/:id/files', verifyToken, async (req, res) => {
  try {
    ...
    if (!canAccessTicket(req, ticket)) {
      return res.status(403).json({ error: 'Not authorized to view these attachments' });
    }
    ...
    const filesWithPreview = await Promise.all((data || []).map(async (file) => {
